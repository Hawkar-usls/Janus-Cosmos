...
def _mask_clean(mask,wcs,ra,dec):
 x,y=_xy(wcs,ra,dec);yy,xx=np.indices(mask.shape);sel=(xx-x)**2+(yy-y)**2<=MASK_RADIUS_PX**2;sel &= np.isfinite(mask)
...
def classify_epoch(image,wcs,mask,mask_wcs,ra,dec,seed):
 ...
 mg=_mask_clean(mask,mask_wcs,ra,dec);gate=psf_relative_injection_recovery_gate(image,seed=seed)
...
def main():
 ...
 if manifest_blob!=MANIFEST_GIT_BLOB_SHA1: raise RuntimeError(f'frozen T2B manifest Git blob mismatch: {manifest_blob}')
 rows=_rows(manifest)
 ...
 for ti,row in enumerate(rows):
  ...
  for ei,e in enumerate('abc'):
   ...
   sci,_=download_source(sci_url,cache,events,target=row['trial_id'],filter_name=f'ZTF_{e}_SCI');msk,_=download_source(msk_url,cache,events,target=row['trial_id'],filter_name=f'ZTF_{e}_MASK')
   ...
   er.update({'science_url':sci_url,'mask_url':msk_url,'science_hdu':sci_hdu,'mask_hdu':mask_hdu,'science_sha256':sha256_file(sci),'mask_sha256':sha256_file(msk)});ep[e]=er
  cls=_classify_trial(ep['a'],ep['b'],ep['c']);diff=None
  if cls=='ISOLATED_B_L0':
   try:
    du=_product_url(row,'b','scimrefdiffimg.fits.fz');dp,_=download_source(du,cache,events,target=row['trial_id'],filter_name='ZTF_B_DIFF');di,dw,dh=_read_image_wcs(dp);x,y=_xy(dw,row['ra_deg'],row['dec_deg']);dt=measure_psf_at(di,y,x)
