...
def panstarrs_query(ra, dec, radius_arcsec):
    ...
    t = Catalogs.query_region(
        f"{ra} {dec}", radius=radius_arcsec * u.arcsec, catalog="Panstarrs", data_release="dr2"
    )
    rows = []
    cols = [
        c
        ...
        try:
            ...
            for i, link in enumerate(links[:8]):
                try:
                    ...
                    downloaded.append(
                        {"file": fn, "url": link, "bytes": len(rr.content), "content_type": ctype}
                    )
...
def run(output_dir: Path, radius_arcsec: float):
    ...
    receipt = {
        "schema": "janus.cosmos.open_sky_scan.v1",
        "role": "SECONDARY_CALIBRATION_AND_CROSS_CHECK",
    ...
    for label, (ra, dec) in TARGETS.items():
        ...
        for name, func in [
            ...
            (
                "2mass_psc",
                lambda: irsa_query("fp_psc", ra, dec, radius_arcsec, ["ra", "dec", "designation", "j_m", "h_m", "k_m"]),
            ),
            (
                "allwise",
                lambda: irsa_query(
        ...
        entry["summary"] = {
            k: {
                "status": v.get("status"),
                "count": v.get("count"),
                "nearest": (v.get("rows") or [None])[0],
            }
            for k, v in entry.items()
            if isinstance(v, dict) and "count" in v
        }
        receipt["targets"][label] = entry
    p = output_dir / "open-sky-scan.json"
...
if __name__ == "__main__":
    ...
    ap.add_argument("--radius-arcsec", type=float, default=120.0)
    a = ap.parse_args()
    run(Path(a.output_dir), a.radius_arcsec)
