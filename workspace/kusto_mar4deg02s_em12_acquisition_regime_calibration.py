...
UA={"User-Agent":"JANUS-KUSTO-MAR4DEG02S-acquisition-regime-calibration/1.0"}
...
pout={
 "artifact_id":"JANUS-KUSTO-MAR4DEG02S-CD169-EM12-ACQUISITION-REGIME-CALIBRATION-RUN-2026-09-22-v1.0",
 "prereg":"data/cousteau/JANUS-KUSTO-CD169-EM12-ACQUISITION-REGIME-CALIBRATION-PREREG-2026-09-22-v1.0.json",
 "target":C,
 "products":product_results,
 "passes_both_2008_lineages":passes_both,
 "product_mixing_signature_pass":positive,
 "verdict":verdict,
 "source_regime_definition_predeclared":True,
 "claim_ceiling":"SOURCE_NATIVE_PROCESSED_PRODUCT_REGIME_CALIBRATION_ONLY"
}
pp=OUT/"JANUS-KUSTO-MAR4DEG02S-CD169-EM12-ACQUISITION-REGIME-CALIBRATION-RUN-2026-09-22-v1.0.json"
pp.write_text(json.dumps(pout,indent=2))
print("\nPRODUCT_DEMIX_DIAGNOSTIC")
print(json.dumps(pout,indent=2))

# Rewrite final calibration verdict according to source-native prereg.
ded="B1-81-1_Acceptl15-22.xyz.ascii"
later="B1-81-1_Acceptl28-33.xyz.ascii"
ded_both=passes_both.get(ded,False)
later_both=passes_both.get(later,False)
later_any_fail=any(not v.get("gate_pass",False) for v in product_results[later]["comparisons"].values())
ded_any_fail=any(not v.get("gate_pass",False) for v in product_results[ded]["comparisons"].values())
if ded_both and later_any_fail:
    regime_verdict="SOURCE_NATIVE_ACQUISITION_REGIME_FIDELITY_DIFFERENCE_SUPPORTED"
elif ded_both and later_both:
    regime_verdict="NO_GLOBAL_LATER_PRODUCT_FAILURE__CAND003_DIFFERENCE_IS_LOCATION_OR_SUPPORT_SPECIFIC"
elif ded_any_fail:
    regime_verdict="NO_EVIDENCE_DEDICATED_MAPPING_PRODUCT_IS_GLOBALLY_MORE_RELIABLE"
else:
    regime_verdict="INSUFFICIENT_PRODUCT_SPECIFIC_SUPPORT"
pout["source_native_regime_verdict"]=regime_verdict
pout["dedicated_mapping_product"]=ded
pout["later_operational_product"]=later
pout["claim_ceiling"]="SOURCE_NATIVE_PROCESSED_PRODUCT_REGIME_CALIBRATION_ONLY"
pp.write_text(json.dumps(pout,indent=2))
print("\nSOURCE_NATIVE_REGIME_CALIBRATION")
print(json.dumps({"passes_both":passes_both,"verdict":regime_verdict,"products":product_results},indent=2))
