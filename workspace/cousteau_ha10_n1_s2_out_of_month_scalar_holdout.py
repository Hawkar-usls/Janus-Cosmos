...
PROTOCOL = DATA / "JANUS-ECHO-COUSTEAU-HA10-N1-S2-OUT-OF-MONTH-SCALAR-HOLDOUT-PROTOCOL-2026-08-22-v1.0.json"
PRIOR_SUMMARY = DATA / "JANUS-ECHO-COUSTEAU-HA10-N1-S2-INDEPENDENT-SCALAR-COLLAPSE-HOLDOUT-RUN-001-SUMMARY-2026-08-22-v1.0.json"
...
EXPECTED_PROTOCOL_BLOB = "594786129fc8961fc47a65f0ded3046552137d99"
EXPECTED_PRIOR_SUMMARY_BLOB = "6b2c075396b3fc60df3fafe41024a72c86a3afff"
...
EXPECTED_PRIOR_VERDICT = "SCALAR_COLLAPSE_HOLDOUT_PASS"
...
def ensure_noncanonical_output(path: Path) -> None:
    try:
        ...
        return
    raise RuntimeError("CANONICAL_DATA_WRITE_FORBIDDEN_BY_OUT_OF_MONTH_SCALAR_HOLDOUT")


def verify_frozen_contracts() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    expected = {
        PROTOCOL: EXPECTED_PROTOCOL_BLOB,
        PRIOR_SUMMARY: EXPECTED_PRIOR_SUMMARY_BLOB,
    ...
    upstream = json.loads(PRIOR_SUMMARY.read_text(encoding="utf-8"))
    frozen = json.loads(FROZEN_119.read_text(encoding="utf-8"))

    if protocol.get("status") != "PREREGISTERED_BEFORE_JANUARY_HOLDOUT_WINDOWS_ARE_DOWNLOADED":
        raise RuntimeError("OUT_OF_MONTH_PROTOCOL_NOT_PREREGISTERED")
    if upstream.get("result", {}).get("verdict") != EXPECTED_PRIOR_VERDICT:
        raise RuntimeError("PRIOR_SCALAR_HOLDOUT_VERDICT_BINDING_DRIFT")
    ...
    upstream_scalar = float(upstream["frozen_contract"]["frozen_scalar_db"])
    ...
    fc = protocol["frequency_contract"]
    if fc.get("119hz_or_117_121hz_bins_may_be_used") is not False:
        raise RuntimeError("TARGET_BAND_EXCLUSION_NOT_FROZEN")
    wc = protocol["holdout_window_contract"]
    if len(wc["dates"]) != 20:
        raise RuntimeError("JANUARY_WINDOW_COUNT_DRIFT")
    if wc["dates"][0] != "2015-01-01" or wc["dates"][-1] != "2015-01-20":
        raise RuntimeError("JANUARY_DATE_BLOCK_DRIFT")
    if wc["window_start_time_utc_each_day"] != "18:00:00Z":
        raise RuntimeError("JANUARY_WINDOW_CLOCK_DRIFT")
    ...
    return protocol, upstream, frozen


def map_verdict(prior_verdict: str) -> str:
    mapping = {
        "BLOCKED_SCALAR_COLLAPSE_HOLDOUT_DATA_ACCESS": "BLOCKED_OUT_OF_MONTH_SCALAR_HOLDOUT_DATA_ACCESS",
        "SCALAR_COLLAPSE_HOLDOUT_PASS": "OUT_OF_MONTH_SCALAR_HOLDOUT_PASS",
        "SCALAR_COLLAPSE_HOLDOUT_FAIL": "OUT_OF_MONTH_SCALAR_HOLDOUT_FAIL",
    }
...
def run(output: Path) -> dict[str, Any]:
    ...
    protocol, upstream, frozen = verify_frozen_contracts()
    ...
    session.headers["User-Agent"] = (
        "Janus-Echo-Cousteau/1.0 preregistered H10 out-of-month scalar holdout"
    )
    ...
    for date_text in wc["dates"]:
        ...
        for cid in channels:
            try:
                ...
                band_powers = prior.powers_for_bands(
                    trace,
                    inventories[cid],
                    bands=bands,
                    nperseg=nperseg,
        ...
        if n1.get("data_status") == "ANALYZED" and s2.get("data_status") == "ANALYZED":
            ...
            row["subbands"] = {}
            for name in bands:
                n1_power = n1["band_powers"][name]
                s2_power = s2["band_powers"][name]
                raw_ratio = prior.ratio_db(
                    float(s2_power["raw_integrated_power"]),
                    float(n1_power["raw_integrated_power"]),
                )
                corrected_ratio = prior.ratio_db(
                    float(s2_power["corrected_integrated_power_pa2"]),
                    float(n1_power["corrected_integrated_power_pa2"]),
                )
                result = {
                    "band_hz": n1_power["band_hz"],
                    "raw_s2_minus_n1_db": raw_ratio,
                    "corrected_s2_minus_n1_db": corrected_ratio,
    ...
    receipt = {
        "artifact_id": "JANUS-ECHO-COUSTEAU-HA10-N1-S2-OUT-OF-MONTH-SCALAR-HOLDOUT-RUN",
        "created_utc": prior.utc_now(),
        "gate_id": protocol["gate_id"],
        "protocol_git_blob_sha1": EXPECTED_PROTOCOL_BLOB,
        "prior_scalar_holdout_summary_git_blob_sha1": EXPECTED_PRIOR_SUMMARY_BLOB,
        "prior_scalar_holdout_verdict": upstream["result"]["verdict"],
        "frozen_119hz_result_git_blob_sha1": EXPECTED_FROZEN_119_BLOB,
        "frozen_119hz_verdict": frozen["summary"]["verdict"],
        "prior_holdout_runner_git_blob_sha1": EXPECTED_PRIOR_RUNNER_BLOB,
        "bounded_transport_v2_git_blob_sha1": EXPECTED_TRANSPORT_V2_BLOB,
        "authority_delta_for_119hz": 0,
        "target_identity": "UNCONFIRMED",
        "target_evidence_delta": "NONE_FROM_OUT_OF_MONTH_SCALAR_HOLDOUT",
