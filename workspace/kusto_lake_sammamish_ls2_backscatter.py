...
SRC=json.loads((ROOT/"data/cousteau/JANUS-KUSTO-LAKE-SAMMAMISH-LS0-SOURCE-BINDING-RECEIPT-2026-09-23-v1.0.json").read_text())
CAND_AUTH=json.loads((ROOT/"data/cousteau/JANUS-KUSTO-LAKE-SAMMAMISH-LS1-BLIND-BATHYMETRY-RECEIPT-2026-09-23-v1.0.json").read_text())
IMP=json.loads((ROOT/"data/cousteau/JANUS-KUSTO-LAKE-SAMMAMISH-LS2-BACKSCATTER-IMPLEMENTATION-ADDENDUM-2026-09-23-v1.0.json").read_text())

UA={"User-Agent":"JANUS-KUSTO-LakeSammamish-LS2-unseen-backscatter/1.0"}
...
NOMINAL_CELL=float(IMP["georef_rule"]["nominal_cell_m"])
REL_TOL=float(IMP["georef_rule"]["relative_resolution_tolerance"])
...
def load_backscatter():
    spec=SRC["backscatter"]
    blob=get(spec["url"]).content
    md5=hashlib.md5(blob).hexdigest();sha=hashlib.sha256(blob).hexdigest()
    if md5.lower()!=spec["md5"].lower():raise RuntimeError("backscatter MD5 mismatch")
    if len(blob)!=int(spec["bytes"]):raise RuntimeError("backscatter byte length mismatch")
    with zipfile.ZipFile(io.BytesIO(blob)) as zf:
        names=zf.namelist()
    ...
    def within_nominal(res):
        return max(abs(res[0]-NOMINAL_CELL),abs(res[1]-NOMINAL_CELL))/NOMINAL_CELL<=REL_TOL
    emb_ok=(not emb.is_identity and within_nominal(eres))
    wvals=None
    if emb_ok:
    ...
    if not within_nominal(res):
        raise RuntimeError(f"effective resolution outside frozen tolerance: {res}")
    ...
    return {
      ...
      "members":names,"world_values":wvals,"embedded_transform":list(emb)[:6],
      "effective_transform":list(transform)[:6],"georef_source":geo,
      "crs":crs,"nodata":nodata,"dtype":dtype,"resolution_m":list(res),"cell_eff_m":cell_eff
    }
...
out={
 "artifact_id":"JANUS-KUSTO-LAKE-SAMMAMISH-LS2-UNSEEN-BACKSCATTER-REPLAY-RUN-2026-09-23-v1.0",
 ...
 "backscatter_input":{
   "url":SRC["backscatter"]["url"],"zip_bytes":len(bs["blob"]),"zip_md5_verified":bs["md5"],
   "zip_sha256":bs["sha256"],"tif_name":bs["tif_name"],"tfw_name":bs["tfw_name"],
...
p=OUT/"JANUS-KUSTO-LAKE-SAMMAMISH-LS2-UNSEEN-BACKSCATTER-REPLAY-RUN-2026-09-23-v1.0.json"
