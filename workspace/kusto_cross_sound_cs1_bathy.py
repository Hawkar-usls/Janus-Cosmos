...
UA={"User-Agent":"JANUS-KUSTO-CrossSound-CS1-blind-bathy/1.0"}
CELL=10.0
RADII=[40,80,160,320]
STRIDE=80.0
TAIL=.995
...
def load_bathy():
    url=SRC["products"]["bathymetry"]["item_download_url"]
    blob=get(url)
    if not blob:
        raise RuntimeError("empty bathymetry payload")
    ...
    if blob[:2]==b"PK":
        ...
        with zipfile.ZipFile(io.BytesIO(blob)) as zf:
            ...
            if len(tifs)!=1:
                raise RuntimeError(f"expected exactly one primary TIFF in item bundle, found {tifs}")
            tname=tifs[0]; tif=zf.read(tname)
    elif blob[:4] in (b"II*\\x00",b"MM\\x00*"):
        container="RAW_TIFF"; tif=blob; tname="ITEM_LEVEL_RAW_TIFF"
    else:
        raise RuntimeError(f"unsupported primary payload magic {blob[:16]!r}")
    ...
    if emb.is_identity:
        raise RuntimeError("Cross Sound primary raster has identity geotransform")
    ...
    return blob,z,valid,emb,{
      "payload_url":url,"payload_sha256":sha,"payload_container":container,
      "payload_members":members,"tif_name":tname,
      "shape":list(z.shape),"dtype":dtype,"nodata":nodata,"crs":crs,
      "resolution_m":list(res),"georef_source":"EMBEDDED_TRANSFORM",
      "effective_transform":list(emb)[:6]
    }
...
for radius in RADII:
    print("CS1 radius",radius,flush=True)
...
for q in all_flags:
    ...
    q=dict(q);q["candidate_id"]=f"CROSS_SOUND_BATHY_C{len(kept)+1:03d}";q["blind_rank"]=len(kept)+1
    kept.append(q)
    if len(kept)>=MAX_CAND:break

out={
 "artifact_id":"JANUS-KUSTO-CROSS-SOUND-CS1-BLIND-BATHYMETRY-RUN-2026-09-23-v1.0",
 "prereg":PRE["artifact_id"],"source_receipt":SRC["artifact_id"],
 "input":input_meta,
 "truth_firewall":{"backscatter_zip_downloaded":False,"backscatter_values_read":False,"landslide_or_fault_labels_read":False},
 ...
 "next_gate":"CS2_FROZEN_CANDIDATE_BACKSCATTER_REPLAY",
 "claim_ceiling":"UNSEEN_OPEN_OCEAN_BLIND_BATHYMETRIC_ANOMALY_CANDIDATE_GENERATION_ONLY"
}
p=OUT/"JANUS-KUSTO-CROSS-SOUND-CS1-BLIND-BATHYMETRY-RUN-2026-09-23-v1.0.json"
