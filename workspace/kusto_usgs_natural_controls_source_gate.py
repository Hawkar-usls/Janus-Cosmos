...
def file_role(name):
 n=name.lower()
 roles=[]
 if any(x in n for x in ["bathy","bathym","dem","depth","elevation"]):roles.append("BATHYMETRY")
 if any(x in n for x in ["backscatter","back_scatter","reflectivity","reflect"]):roles.append("BACKSCATTER")
 if any(x in n for x in [".xml",".txt","metadata","readme","fgdc"]):roles.append("METADATA_OR_TEXT")
 if any(x in n for x in [".tif",".tiff",".asc",".grd",".xyz",".nc",".zip"]):roles.append("NUMERIC_RASTER_OR_ARCHIVE_PRODUCT")
 return roles
...
def children(parent_id):
 # ScienceBase catalog supports parentId query; keep a fallback filter form because
 # old releases have been served through more than one query style over time.
 attempts=[
 ...
 for mode,params in attempts:
  try:
   ...
   audit.append({"mode":mode,"count":len(items)})
   # Strictly keep only children whose metadata actually declares this parent
   exact=[x for x in items if str(x.get("parentId") or "")==str(parent_id)]
   if exact:return exact,audit
...
def crawl(root_id,max_depth=3):
 ...
 while queue:
  ...
  if iid in seen or depth>max_depth:continue
  seen.add(iid)
  url=f"https://www.sciencebase.gov/catalog/item/{iid}?format=json"
  j=getj(url)
  items.append(j)
  if j.get("hasChildren") or not (j.get("files") or []):
   ...
   for k in kids:
    kid=k.get("id")
    if kid and kid not in seen:queue.append((kid,depth+1))
 ...
 try:
  ...
  for item in items:
   ...
   item_summaries.append({
    "id":iid,"title":title,"parentId":item.get("parentId"),
    "hasChildren":item.get("hasChildren"),"files":len(fs)
   })
  outsets.append({
   **ds,
   ...
   "all_files":files,
   "status":"BOUND_WITH_CHILD_TRAVERSAL" if len(items)>1 else ("BOUND" if files else "BOUND_NO_FILES_LISTED")
  })
 except Exception as e:
  outsets.append({**ds,"sciencebase_api":root_url,"status":"ERROR","error":type(e).__name__+": "+str(e)})

out={
 "artifact_id":"JANUS-KUSTO-USGS-NATURAL-CONTROLS-SOURCE-GATE-RUN-2026-09-23-v1.1",
 "supersedes":"JANUS-KUSTO-USGS-NATURAL-CONTROLS-SOURCE-GATE-RUN-2026-09-23-v1.0",
...
print(json.dumps({
 ...
 "datasets":[{
  "id":d["id"],"status":d["status"],"items_traversed":d.get("items_traversed"),"files_total":d.get("files_total"),
