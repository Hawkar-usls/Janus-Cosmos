...
UA={"User-Agent":"JANUS-KUSTO-SouthwestCorner-SWC1-blind-bathy/1.0"}
CELL=5.0
RADII=[20,40,80,160]
STRIDE=40.0
TAIL=.995
...
def load_bathy():
    arc=SRC["bathymetry_archive"]
    blob=get(arc["url"])
    sha=hashlib.sha256(blob).hexdigest()
    if sha.lower()!=arc["sha256"].lower():
        raise RuntimeError("bathymetry archive SHA256 mismatch")
    if len(blob)!=int(arc["bytes"]):
        raise RuntimeError("bathymetry archive byte length mismatch")
    member=arc["member"]["name"]
    with zipfile.ZipFile(io.BytesIO(blob)) as zf:
        names=zf.namelist()
        if member not in names:
            raise RuntimeError(f"frozen bathymetry member missing: {member}")
        zi=zf.getinfo(member)
        if int(zi.file_size)!=int(arc["member"]["uncompressed_bytes"]):
            raise RuntimeError("bathymetry member size mismatch")
        if int(zi.CRC)!=int(arc["member"]["crc32"]):
            raise RuntimeError("bathymetry member CRC mismatch")
        tif=zf.read(member)
    ...
    return blob,z,valid,transform,{
      ...
      "backscatter_values_decoded":False
    }
...
for radius in RADII:
    print("SWC1 radius",radius,flush=True)
...
for q in all_flags:
    ...
    q=dict(q);q["candidate_id"]=f"SWC_BATHY_C{len(kept)+1:03d}";q["blind_rank"]=len(kept)+1
    kept.append(q)
    if len(kept)>=MAX_CAND:break

out={
 "artifact_id":"JANUS-KUSTO-SOUTHWEST-CORNER-SWC1-BLIND-BATHYMETRY-RUN-2026-09-24-v1.0",
 "prereg":PRE["artifact_id"],"source_receipt":SRC["artifact_id"],
 "input":input_meta,
 "truth_firewall":{"backscatter_archive_downloaded":False,"backscatter_values_read":False,"underwater_imagery_read":False,"benthic_interpretation_read":False},
 ...
 "next_gate":"SWC2_FROZEN_CANDIDATE_BACKSCATTER_REPLAY_V2",
 "claim_ceiling":"UNSEEN_WESTERN_AUSTRALIAN_MARINE_BLIND_BATHYMETRIC_ANOMALY_CANDIDATE_GENERATION_ONLY"
}
p=OUT/"JANUS-KUSTO-SOUTHWEST-CORNER-SWC1-BLIND-BATHYMETRY-RUN-2026-09-24-v1.0.json"
