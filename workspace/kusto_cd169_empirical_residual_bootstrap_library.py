...
# --- Frozen extreme-beam vs independent-2008 residual stratification ---
BINS=[
 ("INNER",0.0,3000.0),
 ("MID",3000.0,4500.0),
 ("OUTER",4500.0,5500.0),
...
def residual_stats(vals):
    vals=np.asarray(vals,float)
    if len(vals)==0:
        return {"support_cell_count":0}
    av=np.abs(vals)
    return {
      "support_cell_count":int(len(vals)),
      "median_signed_residual_m":float(np.median(vals)),
      "median_absolute_residual_m":float(np.median(av)),
      "p90_absolute_residual_m":float(np.percentile(av,90)),
      "fraction_abs_residual_ge_50m":float(np.mean(av>=50)),
      "fraction_abs_residual_ge_100m":float(np.mean(av>=100)),
      "fraction_raw_deeper_than_2008":float(np.mean(vals>0)),
      "fraction_raw_deeper_by_ge_100m":float(np.mean(vals>=100))
    }

strat={}
for rawlab in ["LINE18_ERA","LINE32_ERA"]:
    strat[rawlab]={}
    for survlab in SURVEYS:
        common=sorted(set(gr[rawlab]) & set(g2008[survlab]))
        bins={name:[] for name,_,_ in BINS}
        nonext=[]
        signedext=[]
        for k in common:
            rawcell=gr[rawlab][k]; z8=g2008[survlab][k]["z"]
            resid=rawcell["z"]-z8
            aa=rawcell["abs_across_m"]; sa=rawcell["signed_across_m"]
            for name,lo,hi in BINS:
                if lo<=aa<hi:
                    bins[name].append(resid);break
            if aa<5500: nonext.append(resid)
            if (rawlab=="LINE18_ERA" and sa>=5500) or (rawlab=="LINE32_ERA" and sa<=-5500):
                signedext.append(resid)
        outbins={name:residual_stats(v) for name,v in bins.items()}
        ext=outbins["EXTREME"]; contrast=residual_stats(nonext); signed=residual_stats(signedext)
        tailratio=None
        medabsratio=None
        if ext.get("support_cell_count",0) and contrast.get("support_cell_count",0):
            ctail=contrast.get("fraction_abs_residual_ge_100m",0)
            tailratio=(ext.get("fraction_abs_residual_ge_100m",0)/ctail) if ctail>0 else (float("inf") if ext.get("fraction_abs_residual_ge_100m",0)>0 else None)
            cma=contrast.get("median_absolute_residual_m")
            medabsratio=(ext.get("median_absolute_residual_m")/cma) if cma and cma>0 else None
        strat[rawlab][survlab]={
          "total_common_cells":len(common),
          "bins":outbins,
          "nonextreme_abs_across_lt5500":contrast,
          "signed_extreme":signed,
          "extreme_vs_nonextreme_tail_ge100_ratio":tailratio,
          "extreme_vs_nonextreme_median_abs_ratio":medabsratio
        }

primary=strat["LINE32_ERA"]["KN192-07_2008"]
line18p=strat["LINE18_ERA"]["KN192-07_2008"]
support_ok=(primary["signed_extreme"].get("support_cell_count",0)>=30 and line18p["signed_extreme"].get("support_cell_count",0)>=30)

result={
 "artifact_id":"JANUS-KUSTO-CAND003-EXTREME-BEAM-VS-2008-RESIDUAL-STRATIFICATION-RUN-2026-09-23-v1.0",
 "prereg":"data/cousteau/JANUS-KUSTO-CAND003-EXTREME-BEAM-VS-2008-RESIDUAL-STRATIFICATION-PREREG-2026-09-23-v1.0.json",
 "target_center":C,
 "analysis_radius_m":ANALYSIS_R,
 "cell_m":CELL,
 "raw_parser_selftest":raw_parse_counts,
 "stratification":strat,
 "primary_support_ok":support_ok,
 "interpretation_deferred_to_receipt":True,
 "claim_ceiling":"EXTREME_OUTER_BEAM_RESIDUAL_STRATIFICATION_DIAGNOSTIC_ONLY"
}
q=OUT/"JANUS-KUSTO-CAND003-EXTREME-BEAM-VS-2008-RESIDUAL-STRATIFICATION-RUN-2026-09-23-v1.0.json"
q.write_text(json.dumps(result,indent=2))
print("\nEXTREME_BEAM_RESIDUAL_STRATIFICATION")
print(json.dumps({
 "primary_support_ok":support_ok,
 "LINE18":strat["LINE18_ERA"],
 "LINE32":strat["LINE32_ERA"]
},indent=2))


# --- Frozen full empirical residual-vector library ---
def vector_summary(vals):
    ...
    return {
      "support_cell_count":int(len(a)),
      "signed_residual_m":[float(x) for x in a.tolist()],
      "absolute_residual_m":[float(x) for x in aa.tolist()],
      "median_signed_residual_m":float(np.median(a)),
      "median_absolute_residual_m":float(np.median(aa)),
      "p90_absolute_residual_m":float(np.percentile(aa,90)),
      "p95_absolute_residual_m":float(np.percentile(aa,95)),
      "p99_absolute_residual_m":float(np.percentile(aa,99)),
      "fraction_abs_ge_50m":float(np.mean(aa>=50)),
      "fraction_abs_ge_100m":float(np.mean(aa>=100)),
      "fraction_abs_ge_150m":float(np.mean(aa>=150))
    }
...
for rawlab in ["LINE18_ERA","LINE32_ERA"]:
    ...
    for survlab in SURVEYS:
        common=sorted(set(gr[rawlab]) & set(g2008[survlab]))
        bucket={name:[] for name,_,_ in BINS}
        for k in common:
            rc=gr[rawlab][k]
            resid=float(rc["z"]-g2008[survlab][k]["z"])
            aa=float(rc["abs_across_m"])
            for name,lo,hi in BINS:
                if lo<=aa<hi:
                    bucket[name].append(resid)
                    break
        library[rawlab][survlab]={name:vector_summary(vals) for name,vals in bucket.items()}

libout={
  "artifact_id":"JANUS-KUSTO-CD169-EM12-EMPIRICAL-RESIDUAL-BOOTSTRAP-LIBRARY-RUN-2026-09-23-v1.0",
  "prereg":"data/cousteau/JANUS-KUSTO-CD169-EM12-EMPIRICAL-RESIDUAL-BOOTSTRAP-LIBRARY-PREREG-2026-09-23-v1.0.json",
  "source":{
    "raw_zip":ZIP_PATH,
  ...
  "analysis_radius_m":ANALYSIS_R,
  "grid_cell_m":CELL,
  "bins_abs_across_track_m":[
    {"label":"INNER","min":0,"max":3000},
    {"label":"MID","min":3000,"max":4500},
    {"label":"OUTER","min":4500,"max":5500},
    {"label":"EXTREME","min":5500,"max":None}
  ],
  "residual_definition":"raw_CD169_2005_100m_cell_median_depth - independent_2008_100m_cell_median_depth",
  "library":library,
  "sampling_firewall":{
    "CD169_REPLAY":"signed bootstrap allowed",
    "GENERIC_UNSEEN_STRESS":"bootstrap magnitude; randomize sign unless independent signed calibration exists",
    "ADVERSARIAL_REAL_TAILS":"empirical tail stress only; not probability forecast"
  },
  "claim_ceiling":"CD169_EMPIRICAL_MULTIBEAM_RESIDUAL_CALIBRATION_LIBRARY_ONLY__NO_UNSEEN_PREDICTIVE_SKILL"
}
lp=OUT/"JANUS-KUSTO-CD169-EM12-EMPIRICAL-RESIDUAL-BOOTSTRAP-LIBRARY-RUN-2026-09-23-v1.0.json"
lp.write_text(json.dumps(libout,indent=2))
print("\nEMPIRICAL_RESIDUAL_BOOTSTRAP_LIBRARY")
print(json.dumps({
  raw:{surv:{b:{
    "n":library[raw][surv][b]["support_cell_count"],
    "median_abs":library[raw][surv][b].get("median_absolute_residual_m"),
    "p90_abs":library[raw][surv][b].get("p90_absolute_residual_m"),
    "p99_abs":library[raw][surv][b].get("p99_absolute_residual_m"),
    "frac_ge100":library[raw][surv][b].get("fraction_abs_ge_100m")
  } for b in ["INNER","MID","OUTER","EXTREME"]} for surv in SURVEYS} for raw in ["LINE18_ERA","LINE32_ERA"]
},indent=2))
