...
# --- preregistered translational georegistration diagnostic ---
def shift_compare(label, reference_thin, target_pts, dx, dy):
    shifted=reference_thin.copy()
    shifted[:,0]+=dx
    shifted[:,1]+=dy
    ref,zt=support_match(shifted,target_pts,100)
    out={"dx_east_m":dx,"dy_north_m":dy,"matched_point_count":int(len(ref))}
    if len(ref)<50:
        out.update({"eligible":False,"pearson":None,"spearman":None,"sign":None})
        return out
    cx,cy=cxy()
    x=ref[:,0]-cx; y=ref[:,1]-cy
    zr=ref[:,2]
    ...
    spear=float(spearmanr(rr,rt).statistic) if len(rr)>=3 else None
    sign=float(np.mean(np.sign(rr)==np.sign(rt)))
    out.update({"eligible":True,"pearson":pear,"spearman":spear,"sign":sign})
    return out

diag={}
for lab in ["KN192-07_2008","KNOX15RR_2008"]:
    rows=[]
    for dx in range(-500,501,50):
        for dy in range(-500,501,50):
            rows.append(shift_compare(lab,reference_thin,loaded[lab],dx,dy))
    eligible=[r for r in rows if r["eligible"] and r["pearson"] is not None and r["spearman"] is not None]
    eligible.sort(key=lambda r:(-r["pearson"],-r["spearman"],math.hypot(r["dx_east_m"],r["dy_north_m"]),r["dx_east_m"],r["dy_north_m"]))
    best=eligible[0] if eligible else None
    zero=next((r for r in rows if r["dx_east_m"]==0 and r["dy_north_m"]==0),None)
    diag[lab]={
      "zero_shift":zero,
      "best_shift":best,
      "pearson_gain_best_minus_zero":None if not best or not zero or zero["pearson"] is None else best["pearson"]-zero["pearson"],
      "top10":eligible[:10],
      "tested_shift_count":len(rows),
      "eligible_shift_count":len(eligible)
    }

a=diag["KN192-07_2008"]["best_shift"]
b=diag["KNOX15RR_2008"]["best_shift"]
vecdiff=None if not a or not b else math.hypot(a["dx_east_m"]-b["dx_east_m"],a["dy_north_m"]-b["dy_north_m"])
positive=bool(
 a and b and
 a["pearson"]>=0.60 and b["pearson"]>=0.60 and
 diag["KN192-07_2008"]["pearson_gain_best_minus_zero"] is not None and
 diag["KNOX15RR_2008"]["pearson_gain_best_minus_zero"] is not None and
 diag["KN192-07_2008"]["pearson_gain_best_minus_zero"]>=0.15 and
 diag["KNOX15RR_2008"]["pearson_gain_best_minus_zero"]>=0.15 and
 vecdiff is not None and vecdiff<=100
)
dout={
 "artifact_id":"JANUS-KUSTO-CAND003-CD169-EM12-TRANSLATION-DIAGNOSTIC-RUN-2026-09-22-v1.0",
 "prereg":"data/cousteau/JANUS-KUSTO-CAND003-CD169-EM12-TRANSLATION-DIAGNOSTIC-PREREG-2026-09-22-v1.0.json",
 "target":C,
 "reference_support_points":int(len(reference_thin)),
 "diagnostics":diag,
 "best_shift_vector_difference_m":vecdiff,
 "diagnostic_signature_pass":positive,
 "verdict":"CONSISTENT_TRANSLATIONAL_GEOREGISTRATION_SIGNATURE" if positive else "NO_CONSISTENT_TRANSLATIONAL_GEOREGISTRATION_SIGNATURE",
 "strict_temporal_fail_immutable":True,
 "look_elsewhere_warning":"Best-of-441 shift optimization is diagnostic only and cannot count as morphology replication.",
 "claim_ceiling":"TRANSLATIONAL_GEOREGISTRATION_DIAGNOSTIC_ONLY"
}
dp=OUT/"JANUS-KUSTO-CAND003-CD169-EM12-TRANSLATION-DIAGNOSTIC-RUN-2026-09-22-v1.0.json"
dp.write_text(json.dumps(dout,indent=2))
print("\nTRANSLATION_DIAGNOSTIC")
print(json.dumps({
 "KN192-07_2008":diag["KN192-07_2008"],
 "KNOX15RR_2008":diag["KNOX15RR_2008"],
 "best_shift_vector_difference_m":vecdiff,
 "diagnostic_signature_pass":positive,
 "verdict":dout["verdict"]
},indent=2))
