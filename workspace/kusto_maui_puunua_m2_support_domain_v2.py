...
SRC = json.loads((ROOT / "data/cousteau/JANUS-KUSTO-MAUI-PUUNUA-M0-SOURCE-BINDING-RECEIPT-2026-09-23-v1.0.json").read_text())
CAND_AUTH = json.loads((ROOT / "data/cousteau/JANUS-KUSTO-MAUI-PUUNUA-M1-BLIND-BATHYMETRY-RECEIPT-2026-09-23-v1.0.json").read_text())
IMP = json.loads((ROOT / "data/cousteau/JANUS-KUSTO-MAUI-PUUNUA-M2-PAIRED-SUPPORT-DOMAIN-V2-IMPLEMENTATION-ADDENDUM-2026-09-23-v1.0.json").read_text())

UA = {"User-Agent": "JANUS-KUSTO-Maui-Puunua-M2-support-domain-v2/1.0"}
...
def load_raster(kind):
    ...
    with zipfile.ZipFile(io.BytesIO(blob)) as zf:
        ...
        if len(tifs) != 1:
            raise RuntimeError(f"{kind}: expected one TIFF, found {tifs}")
        if len(tfws) > 1:
            raise RuntimeError(f"{kind}: expected <=1 world file, found {tfws}")
        tif = zf.read(tifs[0])
        tfw = zf.read(tfws[0]).decode("utf-8", errors="replace") if tfws else None
    ...
    if emb_ok:
        transform = emb
        geo = "EMBEDDED_TRANSFORM"
    else:
        if tfw is None:
            raise RuntimeError(f"{kind}: no valid embedded transform and no world file")
        wvals, transform = world_affine(tfw)
        geo = "WORLD_FILE"
    res = (abs(float(transform.a)), abs(float(transform.e)))
    if not within_nominal(res):
        raise RuntimeError(f"{kind}: effective resolution outside frozen tolerance: {res}")
    return {
        "data": data,
        "valid": native_valid,
        "transform": transform,
        ...
        "members": names,
        "tif_name": tifs[0],
        "tfw_name": tfws[0] if tfws else None,
        "embedded_transform": list(emb)[:6],
...
if blocked:
    ...
    interpretation = "PASS_PREREGISTERED_UNSEEN_MARINE_PAIRED_SUPPORT_DOMAIN_V2_VALIDATION"
else:
    interpretation = "NO_PREREGISTERED_UNSEEN_MARINE_PAIRED_SUPPORT_DOMAIN_V2_VALIDATION"

def meta(r):
    return {
        "url": SRC["files"]["bathymetry" if r is bathy else "backscatter"]["url"],
        "zip_bytes": len(r["blob"]),
        "zip_md5": r["md5"],
        "zip_sha256": r["sha256"],
        "tif_name": r["tif_name"],
        "tfw_name": r["tfw_name"],
...
out = {
    "artifact_id": "JANUS-KUSTO-MAUI-PUUNUA-M2-UNSEEN-BACKSCATTER-V2-RUN-2026-09-23-v1.0",
    ...
    "truth_firewall": {
        "benthic_or_geologic_labels_read": False,
        "candidate_coordinates_changed": False,
        "candidate_radii_changed": False,
    },
    "interpretation": interpretation,
    "claim_ceiling": "PREREGISTERED_UNSEEN_MARINE_SAME_SURVEY_CROSSCHANNEL_V2_VALIDATION_ONLY__NO_INDEPENDENT_SURVEY_REPLICATION",
}

p = OUT / "JANUS-KUSTO-MAUI-PUUNUA-M2-UNSEEN-BACKSCATTER-V2-RUN-2026-09-23-v1.0.json"
