...
UA={"User-Agent":"JANUS-KUSTO-LakeCrescent-backscatter-replay/1.0"}
...
def get_backscatter():
    ...
    return blob,sha,tifs[0],tif_path,names
...
back_blob,back_sha,tif_name,tif_path,zip_members=get_backscatter()
with rasterio.open(tif_path) as src:
    ...
    if max(abs(res[0]-3.0),abs(res[1]-3.0))>1e-6:
        raise RuntimeError(f"Unexpected backscatter cell size {res}")

    cand_records=[]
    by_radius_candidates=defaultdict(list)
    for c in candidates:
        row,col=src.index(float(c["x_utm10_nad83_m"]),float(c["y_utm10_nad83_m"]))
...
for radius in sorted(by_radius_candidates):
    ...
    scale=np.maximum(1e-12,1.4826*mad)
    cz=np.maximum(0.0,(control_mat-med)/scale)
    ...
    kz=np.maximum(0.0,(cand_mat-med)/scale)
    kscore=0.5*np.max(kz,axis=1)+0.5*np.mean(kz,axis=1)

    for q,zv,sc in zip(cs,kz,kscore):
        q["metric_positive_robust_z"]={m:float(zv[i]) for i,m in enumerate(metric_names)}
...
for _ in range(iters):
    ...
    for radius in sorted(by_radius_candidates):
        ...
        med=np.array([rm["control_metric_median"][m] for m in metric_names])
        mad=np.array([rm["control_metric_mad"][m] for m in metric_names])
        scale=np.maximum(1e-12,1.4826*mad)
        chosen=RNG.sample(pool,n)
        mat=np.array([[q["metrics"][m] for m in metric_names] for q in chosen],dtype=np.float64)
...
pass_primary=bool(scored_candidate_count>0 and perm_p<float(PRE["population_test"]["alpha"]) and observed>float(np.mean(all_control_scores)))

out={
    "artifact_id":"JANUS-KUSTO-LAKE-CRESCENT-3M-BACKSCATTER-REPLAY-RUN-2026-09-23-v1.0",
...
p=OUT/"JANUS-KUSTO-LAKE-CRESCENT-3M-BACKSCATTER-REPLAY-RUN-2026-09-23-v1.0.json"
