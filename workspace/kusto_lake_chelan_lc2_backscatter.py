...
CAND_AUTH=json.loads((ROOT/"data/cousteau/JANUS-KUSTO-LAKE-CHELAN-LC1-BLIND-BATHYMETRY-RECEIPT-2026-09-23-v1.0.json").read_text())
IMP=json.loads((ROOT/"data/cousteau/JANUS-KUSTO-LAKE-CHELAN-LC2-BACKSCATTER-IMPLEMENTATION-ADDENDUM-2026-09-23-v1.0.json").read_text())

UA={"User-Agent":"JANUS-KUSTO-LakeChelan-LC2-unseen-backscatter/1.0"}
...
EXPECTED_CANDIDATES=int(CAND_AUTH["totals"]["frozen_candidates"])
CELL=3.0
MIN_VALID=float(IMP["method_transfer"]["minimum_patch_valid_fraction"])
RNG=random.Random(int(PRE["method_transfer"]["backscatter_replay"]["control_rng_seed"]))
METRICS=list(IMP["method_transfer"]["metrics"])
CONTROLS_PER=int(IMP["method_transfer"]["controls_per_candidate"])
MIN_CONTROLS=int(IMP["method_transfer"]["minimum_controls_per_tile_radius"])
MIN_DIST=float(IMP["method_transfer"]["minimum_distance_from_any_frozen_candidate_m"])
MIN_ACTIVE=int(IMP["method_transfer"]["minimum_active_metrics_per_stratum"])
ITERS=int(IMP["population_test"]["iterations"])
ALPHA=float(IMP["population_test"]["alpha"])
...
def get_candidate_json():
    ...
    j=json.loads(raw)
    if int(j["frozen_candidate_total"])!=EXPECTED_CANDIDATES:
...
def world_affine(text):
    ...
    if len(vals)!=6: raise RuntimeError(f"world file requires six values, got {vals}")
    A,D,B,E,C,F=vals
    return vals,Affine(A,B,C-0.5*A-0.5*B,D,E,F-0.5*D-0.5*E)

def load_backscatter(tile):
    spec=SRC["files"][tile]["backscatter"]
    blob=get(spec["url"]).content
    md5=hashlib.md5(blob).hexdigest()
    sha=hashlib.sha256(blob).hexdigest()
    if md5.lower()!=spec["md5"].lower():
        raise RuntimeError(f"{tile}: backscatter MD5 mismatch {md5} != {spec['md5']}")
    if len(blob)!=int(spec["bytes"]):
        raise RuntimeError(f"{tile}: backscatter byte mismatch {len(blob)} != {spec['bytes']}")
    with zipfile.ZipFile(io.BytesIO(blob)) as zf:
        names=zf.namelist()
        ...
        if len(tifs)!=1: raise RuntimeError(f"{tile}: expected one TIFF, found {tifs}")
        if len(tfws)>1: raise RuntimeError(f"{tile}: expected <=1 world file, found {tfws}")
    ...
    if emb_ok:
        ...
        if tfw is None: raise RuntimeError(f"{tile}: no usable embedded transform and no world file")
        vals,transform=world_affine(tfw); geo="WORLD_FILE"
    res=(abs(float(transform.a)),abs(float(transform.e)))
    if max(abs(res[0]-CELL),abs(res[1]-CELL))>1e-6:
        raise RuntimeError(f"{tile}: unexpected effective resolution {res}")
    return {
      "data":data,"valid":valid,"transform":transform,"blob":blob,
      "sha256":sha,"md5":md5,"tif_name":tifs[0],"tfw_name":tfws[0] if tfws else None,
      "members":names,"world_values":vals,"embedded_transform":list(emb)[:6],
      "effective_transform":list(transform)[:6],"georef_source":geo,
      "crs":crs,"nodata":nodata,"dtype":dtype,"resolution_m":list(res)
    }
...
for c in candidates: by_tile[str(c["tile"])].append(c)
...
for tile in sorted(by_tile):
    print("LC2 TILE",tile,flush=True)
    bs=load_backscatter(tile)
    arr=bs["data"]; valid=bs["valid"]; transform=bs["transform"]
    tile_candidates=by_tile[tile]
    frozen_xy=np.array([[float(c["x_m"]),float(c["y_m"])] for c in tile_candidates],dtype=float)

    # Coordinate-system sanity: candidate centers must map into this paired backscatter tile if covered.
    tile_by_radius=defaultdict(list)
    ...
    input_meta[tile]={
      "url":SRC["files"][tile]["backscatter"]["url"],
      "zip_bytes":len(bs["blob"]),"zip_md5_verified":bs["md5"],"zip_sha256":bs["sha256"],
      "tif_name":bs["tif_name"],"tfw_name":bs["tfw_name"],"zip_members":bs["members"],
      "shape":list(arr.shape),"dtype":bs["dtype"],"nodata":bs["nodata"],
      "valid_pixels":int(np.sum(valid)),"resolution_m":bs["resolution_m"],
      ...
      "world_file_values":bs["world_values"]
    }

    for radius in sorted(tile_by_radius):
        key=f"{tile}|{radius}"
        frozen=tile_by_radius[radius]
        scored=[q for q in frozen if q["metrics"] is not None]
        if not scored:
            strata[key]={
              "tile":tile,"radius_m":radius,"frozen_candidate_count":len(frozen),
              "scored_candidate_count":0,"status":"COVERAGE_BLOCKED",
              "active_metrics":[],"excluded_zero_mad_metrics":[],
              "candidate_support_count":0
            }
            blocked_strata.append({"stratum":key,"reason":"COVERAGE_BLOCKED"})
            continue

        target=max(MIN_CONTROLS,len(scored)*CONTROLS_PER)
        controls=[]; tries=0
        rpx=max(1,int(round(radius/CELL))); outer=2*rpx
        h,w=arr.shape
        ...
        if len(controls)<target:
            raise RuntimeError(f"{key}: controls {len(controls)}/{target}")
        ...
        if len(active)<MIN_ACTIVE:
            strata[key]={
              "tile":tile,"radius_m":radius,"frozen_candidate_count":len(frozen),
              "scored_candidate_count":len(scored),"control_count":len(controls),
            ...
            blocked_strata.append({"stratum":key,"reason":"DEGENERATE_METRICS_BLOCKED"})
            continue
    ...
    # release large tile raster before next tile
    del arr,valid,bs
...
if blocked_strata or not scorable:
    perm_p=None
else:
    ge=0
    for _ in range(ITERS):
        ...
        for key in scorable:
            ...
            if n>len(pool): raise RuntimeError(f"{key}: permutation sample n > controls")
...
# Remove internal control score pools from serialized stratum models.
for v in strata.values():v.pop("_control_scores",None)

if blocked_strata:
    interpretation="PRIMARY_VALIDATION_BLOCKED_BY_COVERAGE_OR_DEGENERATE_STRATUM"
elif primary_pass:
    interpretation="PASS_PREREGISTERED_UNSEEN_SAME_SURVEY_CROSS_CHANNEL_VALIDATION"
else:
    interpretation="NO_PREREGISTERED_UNSEEN_SAME_SURVEY_CROSS_CHANNEL_VALIDATION"

out={
 "artifact_id":"JANUS-KUSTO-LAKE-CHELAN-LC2-UNSEEN-BACKSCATTER-REPLAY-RUN-2026-09-23-v1.0",
 ...
 "backscatter_inputs":input_meta,
 "strata":strata,
 "candidate_results":candidate_results,
 "coverage":{
   "frozen_candidate_count":len(candidates),
   "scorable_candidate_count":scored_count,
   "blocked_strata":blocked_strata
 },
 "population":{
 ...
 "truth_firewall":{
   ...
   "delta_locations_read":False,
   "candidate_coordinates_changed":False,
   "candidate_radii_changed":False
 },
 "interpretation":interpretation,
 "claim_ceiling":"PREREGISTERED_UNSEEN_SAME_SURVEY_CROSS_CHANNEL_VALIDATION_ONLY__NO_INDEPENDENT_SURVEY_REPLICATION"
}
p=OUT/"JANUS-KUSTO-LAKE-CHELAN-LC2-UNSEEN-BACKSCATTER-REPLAY-RUN-2026-09-23-v1.0.json"
