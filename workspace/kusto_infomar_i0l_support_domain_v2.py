...
IMP=json.loads((ROOT/"data/cousteau/JANUS-KUSTO-INFOMAR-I0L-UNCHANGED-V2-IMPLEMENTATION-FREEZE-2026-09-24-v1.0.json").read_text())
BR=json.loads((ROOT/"data/cousteau/JANUS-KUSTO-INFOMAR-I0L-WGS84-BACKSCATTER-NATIVE-METRIC-BRIDGE-2026-09-24-v1.0.json").read_text())
...
AUTH=json.loads((ROOT/"data/cousteau/JANUS-KUSTO-INFOMAR-I0K-BLIND-BATHYMETRY-RECEIPT-2026-09-24-v1.0.json").read_text())

UA={"User-Agent":"JANUS-KUSTO-INFOMAR-I0L-v2/1.0","Accept-Encoding":"identity"}
...
CELL=float(json.loads((ROOT/"data/cousteau/JANUS-KUSTO-INFOMAR-I0J-NATIVE-CELL-REPRESENTATION-BRIDGE-2026-09-24-v1.0.json").read_text())["effective_native_cell_m"])
...
def extract_verified(label,spec,expected,tmp):
    zpath=tmp/f"{label}.zip"; tpath=tmp/f"{label}.tif"
    ...
    with zipfile.ZipFile(zpath) as zf:
        ...
        if f"{zi.CRC:08x}".lower()!=str(spec["crc32"]).lower(): raise RuntimeError(f"{label}: CRC drift")
        if int(zi.file_size)!=int(spec["uncompressed_size"]): raise RuntimeError(f"{label}: member size drift")
        with zf.open(zi) as src, open(tpath,"wb") as dst:
            while True:
                b=src.read(1024*1024)
                if not b: break
                dst.write(b); msha.update(b); mbytes+=len(b)
...
with tempfile.TemporaryDirectory(prefix="kusto_infomar_i0l_") as td:
    tmp=Path(td)
    ...
    with rasterio.open(back_path) as ds:
        if str(ds.crs)!="EPSG:4326": raise RuntimeError(f"unexpected backscatter CRS {ds.crs}")
        ma=ds.read(1,masked=True)
    ...
    status=("PASS_PREREGISTERED_UNSEEN_INFOMAR_CB13_02_PAIRED_SUPPORT_DOMAIN_V2_VALIDATION"
            if primary_pass else
            "PRIMARY_VALIDATION_BLOCKED_BY_SUPPORT_COVERAGE_OR_DEGENERATE_STRATUM"
            if blocked else
            "NO_PREREGISTERED_UNSEEN_INFOMAR_CB13_02_PAIRED_SUPPORT_DOMAIN_V2_VALIDATION")

    out={
      "artifact_id":"JANUS-KUSTO-INFOMAR-I0L-CB13_02-UNSEEN-BACKSCATTER-V2-RUN-2026-09-24-v1.0",
      ...
      "claim_ceiling":"PREREGISTERED_UNSEEN_INFOMAR_CB13_02_SAME_SURVEY_CROSSCHANNEL_V2_VALIDATION"
    }
