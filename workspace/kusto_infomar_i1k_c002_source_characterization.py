...
OUT=ROOT/"workspace"/"kusto_global_groundtruth_out"/"i1k_c002"
OUT.mkdir(parents=True,exist_ok=True)
PRE=json.loads((ROOT/"data/cousteau/JANUS-KUSTO-INFOMAR-I1K-C002-TARGET-CENTERED-SOURCE-VALUE-CHARACTERIZATION-PREREG-2026-09-25-v1.0.json").read_text())
IMP=json.loads((ROOT/"data/cousteau/JANUS-KUSTO-INFOMAR-I1K-C002-SOURCE-VALUE-IMPLEMENTATION-FREEZE-2026-09-25-v1.0.json").read_text())
T=IMP["target"]
UA={"User-Agent":"JANUS-KUSTO-I1K-C002/1.0","Accept-Encoding":"identity"}
...
def render_hillshade(a,valid,dx,dy,extent,path,r):
    ...
    ax.scatter([0],[0],marker="+",s=70)
    ax.set_xlabel("metres east of frozen target");ax.set_ylabel("metres north of frozen target")
    ax.set_title("C002 bathymetry hillshade, ±250 m")
    fig.tight_layout();fig.savefig(path,dpi=160);plt.close(fig)
...
with tempfile.TemporaryDirectory(prefix="kusto_i1k_") as td:
    ...
    with rasterio.open(bp) as ds:
        ...
        render(a250,v250,extent,"C002 native bathymetry, ±250 m",OUT/"c002_bathymetry_native_250m.png","viridis",r)
        render_hillshade(a250,v250,dx,dy,extent,OUT/"c002_bathymetry_hillshade_250m.png",r)
    ...
    with rasterio.open(kp) as ds:
        ...
        render(ka,kv,kextent,"C002 native backscatter, ±250 m",OUT/"c002_backscatter_native_250m.png","gray",r)

    out={
      "artifact_id":"JANUS-KUSTO-INFOMAR-I1K-C002-TARGET-CENTERED-SOURCE-VALUE-CHARACTERIZATION-RUN-2026-09-25-v1.0",
      ...
      "claim_ceiling":"POSTRESULT_NATIVE_SOURCE_VALUE_MORPHOLOGY_CHARACTERIZATION_ONLY"
    }
    raw=json.dumps(out,indent=2,ensure_ascii=False)
    (OUT/"JANUS-KUSTO-INFOMAR-I1K-C002-TARGET-CENTERED-SOURCE-VALUE-CHARACTERIZATION-RUN-2026-09-25-v1.0.json").write_text(raw)
    print(json.dumps({
