...
UA={"User-Agent":"JANUS-KUSTO-Maui-Puunua-M1-blind-bathy/1.0"}
CELL=1.0
RADII=[4,8,16,32]
STRIDE=8.0
TAIL=.995
...
for radius in RADII:
    print("M1 radius",radius,flush=True)
...
for q in all_flags:
    ...
    q=dict(q);q["candidate_id"]=f"MAUI_BATHY_C{len(kept)+1:03d}";q["blind_rank"]=len(kept)+1
    kept.append(q)
    if len(kept)>=MAX_CAND:break

out={
 "artifact_id":"JANUS-KUSTO-MAUI-PUUNUA-M1-BLIND-BATHYMETRY-RUN-2026-09-23-v1.0",
 "prereg":PRE["artifact_id"],"source_receipt":SRC["artifact_id"],
 "input":input_meta,
 "truth_firewall":{"backscatter_zip_downloaded":False,"backscatter_values_read":False,"benthic_or_geologic_labels_read":False},
