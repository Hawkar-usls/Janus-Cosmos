...
SRC = json.loads((ROOT / "data/cousteau/JANUS-KUSTO-SHELLHARBOUR-S0-FINAL-PAIRED-SOURCE-RECEIPT-2026-09-23-v1.0.json").read_text())
...
def load_raster(kind):
    if kind == "bathymetry":
        source_url = SRC["bathymetry"]["current_url"]
        archive = get(source_url).content
        archive_sha = hashlib.sha256(archive).hexdigest()
        if archive_sha.lower() != MEM["source_archive"]["sha256"].lower():
            raise RuntimeError("bathymetry source archive SHA mismatch")
        if len(archive) != int(MEM["source_archive"]["bytes"]):
            raise RuntimeError("bathymetry source archive byte length mismatch")
        member = MEM["discovery_member"]["name"]
        with zipfile.ZipFile(io.BytesIO(archive)) as zf:
            names = zf.namelist()
            if member not in names:
                raise RuntimeError("frozen 2m bathymetry member missing")
            zi = zf.getinfo(member)
            if int(zi.file_size) != int(MEM["discovery_member"]["uncompressed_bytes"]):
                raise RuntimeError("frozen bathymetry member size mismatch")
            if int(zi.CRC) != int(MEM["discovery_member"]["crc32"]):
                raise RuntimeError("frozen bathymetry member CRC mismatch")
            tif = zf.read(member)
        r = _decode_tif(tif, kind, 2.0, source_url, archive_sha, names, member)
...
if blocked:
    ...
    interpretation = "PASS_PREREGISTERED_UNSEEN_AUSTRALIAN_MARINE_PAIRED_SUPPORT_DOMAIN_V2_VALIDATION"
else:
    interpretation = "NO_PREREGISTERED_UNSEEN_AUSTRALIAN_MARINE_PAIRED_SUPPORT_DOMAIN_V2_VALIDATION"
...
out = {
    "artifact_id": "JANUS-KUSTO-SHELLHARBOUR-S2-UNSEEN-BACKSCATTER-V2-RUN-2026-09-23-v1.0",
    ...
    "claim_ceiling": "PREREGISTERED_UNSEEN_AUSTRALIAN_MARINE_SAME_SURVEY_CROSSCHANNEL_V2_VALIDATION_ONLY__NO_INDEPENDENT_SURVEY_REPLICATION",
}

p = OUT / "JANUS-KUSTO-SHELLHARBOUR-S2-UNSEEN-BACKSCATTER-V2-RUN-2026-09-23-v1.0.json"
