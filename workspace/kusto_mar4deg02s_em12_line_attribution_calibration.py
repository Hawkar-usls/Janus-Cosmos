...
C={"id":"MAR_4DEG02S_PUBLISHED_REGION_CAL","lat":-4.03,"lon":-12.25}
...
UA={"User-Agent":"JANUS-KUSTO-MAR4DEG02S-line-attribution-calibration/1.0"}
...
# --- preregistered geometry-only line attribution diagnostic ---
from datetime import datetime, timedelta, timezone

NAV_PATH="/bodc/bodc/data/BODCREQ-9406/CD169_TOBI/sd11281/Nav/cd169leg1_1min.listit"
LINE_WINDOWS={
 15:("2005-02-23T17:37:00Z","2005-02-23T19:37:00Z"),
 16:("2005-02-23T19:37:00Z","2005-02-23T21:30:00Z"),
 17:("2005-02-23T21:30:00Z","2005-02-24T00:02:00Z"),
 18:("2005-02-24T00:02:00Z","2005-02-24T01:59:00Z"),
 19:("2005-02-24T01:59:00Z","2005-02-24T04:11:00Z"),
 20:("2005-02-24T04:11:00Z","2005-02-24T06:00:00Z"),
 21:("2005-02-24T06:00:00Z","2005-02-24T06:25:00Z"),
 22:("2005-02-24T06:25:00Z","2005-02-24T06:35:00Z"),
...
PRODUCT_LINES={
 "B1-81-1_Acceptl15-22.xyz.ascii":list(range(15,23)),
 "B1-81-1_Acceptl28-33.xyz.ascii":list(range(28,34))
}
...
def load_nav():
    f=ftp();chunks=[]
    try:
        f.retrbinary("RETR "+NAV_PATH,chunks.append)
    finally:
        f.close()
    b=b"".join(chunks)
    rows=[]
    ...
    for raw in b.decode("ascii","ignore").splitlines():
        ...
        if len(p)<7:continue
        try:
            yy=int(p[0]);jd=int(p[1]);hh,mm,ss=map(int,p[2].split(":"))
            lat=float(p[3]);lon=float(p[5])
        except:continue
        dt=base+timedelta(days=jd-1,hours=hh,minutes=mm,seconds=ss)
        x,y=gxy(lon,lat)
        rows.append((dt.timestamp(),x,y,lat,lon))
    return rows,hashlib.sha256(b).hexdigest()

def track_for_line(nav,line):
    a,b=LINE_WINDOWS[line];ta,tb=parse_iso(a),parse_iso(b)
    rows=[r for r in nav if ta<=r[0]<=tb]
    return np.asarray([[r[1],r[2]] for r in rows],float)

def point_polyline_distance(px,py,track):
    if len(track)==0:return float("inf")
    if len(track)==1:return float(math.hypot(px-track[0,0],py-track[0,1]))
    a=track[:-1];b=track[1:];v=b-a
    w=np.array([px,py])-a
    den=np.sum(v*v,axis=1)
    t=np.zeros(len(den),float)
    nz=den>0
    t[nz]=np.sum(w[nz]*v[nz],axis=1)/den[nz]
    t=np.clip(t,0,1)
    q=a+t[:,None]*v
    return float(np.min(np.hypot(q[:,0]-px,q[:,1]-py)))

nav,nav_sha=load_nav()
tracks={line:track_for_line(nav,line) for line in LINE_WINDOWS}

line_diag={}
for pname,expected_sha in PRODUCTS.items():
    pts,_=load_cd_product(pname,expected_sha)
    lines=PRODUCT_LINES[pname]
    attributed={line:[] for line in lines}
    distrows={line:[] for line in lines}
    gaps={line:[] for line in lines}
    for row in pts:
        distances=sorted((point_polyline_distance(row[0],row[1],tracks[line]),line) for line in lines)
        d1,line1=distances[0]
        d2=distances[1][0] if len(distances)>1 else float("inf")
        attributed[line1].append(tuple(row))
        distrows[line1].append(d1)
        gaps[line1].append(d2-d1)
    line_diag[pname]={}
    for line in lines:
        arr=np.asarray(attributed[line],float) if attributed[line] else np.empty((0,3),float)
        thin=thin_reference_2005(arr) if len(arr) else np.empty((0,5),float)
        comparisons={}
        for lab in ["KN192-07_2008","KNOX15RR_2008"]:
            if len(thin)<30:
                comparisons[lab]={"matched_point_count":0,"gate_pass":False,"verdict":"INSUFFICIENT_LINE_SPECIFIC_SUPPORT"}
            else:
                comparisons[lab]=compare_product_support(lab,thin,loaded[lab])
        line_diag[pname][str(line)]={
          "line_window_utc":LINE_WINDOWS[line],
          "nav_track_points":int(len(tracks[line])),
          "attributed_raw_points":int(len(arr)),
          "geometry_thinned_support_points":int(len(thin)),
          "median_nearest_ship_track_distance_m":None if not distrows[line] else float(np.median(distrows[line])),
          "median_second_minus_first_track_distance_gap_m":None if not gaps[line] else float(np.median(gaps[line])),
          "comparisons":comparisons,
          "passes_both_2008_lineages":bool(len(thin)>=30 and all(comparisons[x].get("gate_pass",False) for x in comparisons))
        }

pass_lines={}
for pname,lines in line_diag.items():
    pass_lines[pname]=[int(line) for line,r in lines.items() if r["passes_both_2008_lineages"]]

lout={
 "artifact_id":"JANUS-KUSTO-MAR4DEG02S-CD169-EM12-LINE-ATTRIBUTION-CALIBRATION-RUN-2026-09-22-v1.0",
 "prereg":"data/cousteau/JANUS-KUSTO-MAR4DEG02S-CD169-EM12-LINE-ATTRIBUTION-CALIBRATION-PREREG-2026-09-22-v1.0.json",
 "target":C,
 "nav":{"path":NAV_PATH,"sha256":nav_sha,"parsed_rows":len(nav)},
 "line_diagnostics":line_diag,
 "lines_passing_both_2008_lineages":pass_lines,
 "assignment_warning":"Nearest ship-track line attribution is geometry-inferred from processed XYZ positions and 1-minute navigation. It is not raw ping provenance.",
 "known_positive_role_preserved":True,
 "claim_ceiling":"LINE_LEVEL_POSITIVE_CALIBRATION_DIAGNOSTIC_ONLY"
}
lp=OUT/"JANUS-KUSTO-MAR4DEG02S-CD169-EM12-LINE-ATTRIBUTION-CALIBRATION-RUN-2026-09-22-v1.0.json"
lp.write_text(json.dumps(lout,indent=2))
print("\nKNOWN_POSITIVE_LINE_ATTRIBUTION_CALIBRATION")
print(json.dumps({
 "nav":lout["nav"],
 "line_diagnostics":line_diag,
 "lines_passing_both_2008_lineages":pass_lines
